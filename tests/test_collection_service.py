...
from dex_forge.backend.models import HandMode, RecorderState, RecordingDecision, Scenario
...
def test_start_recording_uses_current_prompt(service):
    ...
    recording = service.start_recording(start_time=datetime(2026, 4, 16, 12, 0, 0, tzinfo=UTC))
...
def test_next_prompt_cycles_through_all_prompts_before_repeating(tmp_path):
    scenarios = [
        Scenario(category="grasp", action="power", variation="a", prompt_text="Prompt A"),
        Scenario(category="pinch", action="precision", variation="b", prompt_text="Prompt B"),
        Scenario(category="gesture", action="spread", variation="c", prompt_text="Prompt C"),
    ]
...
def test_stop_writes_recording_outputs_and_accept_persists_review_event(service):
    service.start_collection(active_hands=HandMode.LEFT)
    service.next_prompt()

    service.start_recording(start_time=datetime(2026, 4, 16, 12, 0, 0, tzinfo=UTC))
    ...
    recording = service.stop_recording(stop_time=datetime(2026, 4, 16, 12, 0, 1, tzinfo=UTC))
    ...
    accepted = service.decide_recording(recording.recording_id, RecordingDecision.ACCEPT)
...
def test_same_prompt_accumulates_recordings_under_one_task_folder(service):
    ...
    first_recording = service.stop_recording(stop_time=datetime(2026, 4, 16, 12, 0, 1, tzinfo=UTC))
    service.decide_recording(first_recording.recording_id, RecordingDecision.ACCEPT)

    service.next_prompt()
    service.start_recording(start_time=datetime(2026, 4, 16, 12, 1, 0, tzinfo=UTC))
    service.record_message(
        topic="/teleop/human/hand_left/pose",
        message=make_pose_array(),
        timestamp_ns=2_000_000,
    )
    second_recording = service.stop_recording(stop_time=datetime(2026, 4, 16, 12, 1, 1, tzinfo=UTC))

    assert first_recording.task_id == second_recording.task_id
    assert first_recording.task_id == hashlib.sha256(prompt.prompt_text.encode("utf-8")).hexdigest()
    assert first_recording.recording_dir.parent == second_recording.recording_dir.parent
    ...
    assert not first_recording.recording_dir.joinpath("recording_manifest.json").exists()
    assert first_recording.recording_dir.joinpath("metadata.yaml").exists()

    metadata = yaml.safe_load(first_recording.recording_dir.joinpath("metadata.yaml").read_text())
...
def test_discard_removes_recording_directory(service):
    ...
    discarded = service.decide_recording(recording.recording_id, RecordingDecision.DISCARD)
...
def test_save_and_record_more_keeps_prompt_and_allows_next_recording(service):
    ...
    saved = service.decide_recording(first_recording.recording_id, RecordingDecision.RECORD_MORE)
    ...
    second_recording = service.start_recording(start_time=datetime(2026, 4, 16, 12, 1, 0, tzinfo=UTC))
    assert second_recording.task_id == first_recording.task_id
    assert second_recording.recording_dir.name == "recording_000002"


def test_stop_marks_recording_invalid_when_required_topic_has_no_frames(service):
    service.start_collection(active_hands=HandMode.LEFT)
    service.next_prompt()
    service.start_recording(start_time=datetime(2026, 4, 16, 12, 0, 0, tzinfo=UTC))
    ...
    events = read_event_records(str(recording.recording_dir))
    assert any(event["event_type"] == "sanity_check_failed" for event in events)
...
def test_same_prompt_with_conflicting_label_raises_error(tmp_path):
    scenarios = [
        Scenario(category="grasp", action="power", variation="a", prompt_text="Shared prompt"),
        Scenario(category="pinch", action="precision", variation="b", prompt_text="Shared prompt"),
    ]
    service = CollectionService(
        dataset_root=tmp_path / "dataset",
        scenarios=scenarios,
